...
public class ExternalInfo {
	...
	public static String getExternalProtocol(IDiagramProfile profile) {
		return isEmpty(System.getProperty(EXTERNAL_PROTOCOL)) ? profile
				.getExternalLoadURLProtocol() : System
				.getProperty(EXTERNAL_PROTOCOL);
	}
	...
	public static String getExternalUsr(IDiagramProfile profile) {
		return isEmpty(System.getProperty(EXTERNAL_USR)) ? profile.getUsr()
				: System.getProperty(EXTERNAL_USR);
	}

	public static String getExternalPwd(IDiagramProfile profile) {
		return isEmpty(System.getProperty(EXTERNAL_PWD)) ? profile.getPwd()
				: System.getProperty(EXTERNAL_PWD);
	}

	public static String getExternalRootPath(IDiagramProfile profile) {
		String subdomain = profile.getExternalLoadURLSubdomain();
		subdomain = subdomain.substring(0, subdomain.indexOf("/"));
		String protocol = getExternalProtocol(profile);
		String host = getExternalHost(profile);
		return protocol + "://" + host + "/" + subdomain + "/";
	}

	public static String getExternalPackagesBasePath(IDiagramProfile profile) {
		return getExternalRootPath(profile) + "rest/packages/";
	}
