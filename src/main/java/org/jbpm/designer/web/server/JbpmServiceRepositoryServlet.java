...
public class JbpmServiceRepositoryServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		IDiagramProfile profile = ServletUtil.getProfile(req, profileName,
				getServletContext());
		Map<String, WorkDefinitionImpl> workitemsFromRepo = WorkItemRepository
				.getWorkDefinitions(repoURL);
		if (action != null && action.equalsIgnoreCase(displayRepoContent)) {
			if (workitemsFromRepo != null && workitemsFromRepo.size() > 0) {
				...
				for (String key : workitemsFromRepo.keySet()) {
					...
					keyList.add(wd.getDisplayName() == null ? "" : wd
							.getDisplayName());
					keyList.add(repoURL + "/" + wd.getName() + "/"
							+ wd.getIcon());
					keyList.add(wd.getCategory() == null ? "" : wd
							.getCategory());
					keyList.add(wd.getExplanationText() == null ? "" : wd
							.getExplanationText());
					keyList.add(repoURL + "/" + wd.getName() + "/"
							+ wd.getDocumentation());
				...
				for (Entry<String, List<String>> retMapKey : retMap.entrySet()) {
					try {
						jsonObject
								.put(retMapKey.getKey(), retMapKey.getValue());
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
				...
				&& action.equalsIgnoreCase(installRepoContent)) {
			resp.setCharacterEncoding("UTF-8");
			resp.setContentType("application/json");
			if (workitemsFromRepo != null && workitemsFromRepo.size() > 0) {
				...
				for (String key : workitemsFromRepo.keySet()) {
					if (key.equals(assetsToInstall)
							&& categoryToInstall.equals(workitemsFromRepo.get(
									key).getCategory())) {
						String workitemDefinitionURL = repoURL + "/"
								+ workitemsFromRepo.get(key).getName() + "/"
								+ workitemsFromRepo.get(key).getName() + ".wid";
						String iconFileURL = repoURL + "/"
								+ workitemsFromRepo.get(key).getName() + "/"
								+ workitemsFromRepo.get(key).getIcon();
						String workItemDefinitionContent = ConfFileUtils
								.URLContentsToString(new URL(
										workitemDefinitionURL));
						...
						try {
							...
							_logger.error("Could not read icon image: "
									+ e1.getMessage());
						}
						...
						for (String nextPackage : packageNames) {
							String packageAssetURL = packagesBase
									+ URLEncoder.encode(nextPackage, "UTF-8")
									+ "/assets/";
							try {
								XMLInputFactory factory = XMLInputFactory
										.newInstance();
								InputStream is = ServletUtil
										.getInputStreamForURL(packageAssetURL,
												"GET", profile);
								...
								while (reader.hasNext()) {
									...
									if (next == XMLStreamReader.START_ELEMENT) {
										if ("uuid"
											...
											String eleText = reader
													.getElementText();
											if (uuid.equals(eleText)) {
												pkg = URLEncoder.encode(
														nextPackage, "UTF-8");
												gotPackage = true;
												break;
											}
						...
						if (gotPackage) {
							String epkg = URLEncoder.encode(pkg, "UTF-8");
							String widURL = packagesBase + epkg + "/assets/"
									+ widName + ".wid";
							String iconURL = packagesBase + epkg + "/assets/"
									+ iconName;
							String packageAssetsURL = packagesBase + epkg
									+ "/assets/";
							...
							checkWidConnection.setRequestProperty("Accept",
									"application/atom+xml");
							checkWidConnection.connect();
							_logger.info("check wid connection response code: "
									+ checkWidConnection.getResponseCode());
							if (checkWidConnection.getResponseCode() == 200) {
								...
								_logger.info("delete wid response code: "
							...
							checkIconConnection.setRequestProperty("Accept",
									"application/atom+xml");
							checkIconConnection.connect();
							_logger.info("check icon connection response code: "
									+ checkIconConnection.getResponseCode());
							if (checkIconConnection.getResponseCode() == 200) {
								...
								deleteConnection.setRequestMethod("DELETE");
								deleteConnection.connect();
								_logger.info("delete icon response code: "
										+ deleteConnection.getResponseCode());
							}

							// replace the icon value of the workitem config to
							// include the guvnor rest url
							workItemDefinitionContent = workItemDefinitionContent
									.replaceAll(
											"(\"icon\"\\s*\\:\\s*\")(.*?)(\")",
											"$1"
													+ (packageAssetsURL
							...
							createWidConnection.setRequestProperty(
							...
							createWidConnection.getOutputStream()
									.write(workItemDefinitionContent
											.getBytes("UTF-8"));
							createWidConnection.connect();
							System.out.println("created wid configuration:"
							...
							createIconConnection.setRequestProperty(
									"Content-Type", "application/octet-stream");
							createIconConnection.setRequestProperty("Accept",
									"application/atom+xml");
							createIconConnection.setRequestProperty("Slug",
									URLEncoder.encode(iconName, "UTF-8"));
							createIconConnection.setDoOutput(true);
							createIconConnection.getOutputStream().write(
									iconContent);
							createIconConnection.connect();
							_logger.info("icon creation response code: "
									+ createIconConnection.getResponseCode());
							System.out.println("created icon:"
									+ createIconConnection.getResponseCode());
							resp.setCharacterEncoding("UTF-8");
							resp.setContentType("application/json");
							...
							_logger.error("Could not find the package for uuid: "
									+ uuid);
							resp.setCharacterEncoding("UTF-8");
							resp.setContentType("application/json");
							resp.getWriter().write("false");
