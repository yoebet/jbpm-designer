...
public class ThemeServlet extends HttpServlet {
	...
	private String getThemeJson(IDiagramProfile profile,
		...
		String themesURL = packagesBase + "globalArea/assets/" + THEME_NAME;

		String themesSourceURL = packagesBase + "globalArea/assets/"
				+ THEME_NAME + "/source";
		...
		try {
			...
			if (checkConnection.getResponseCode() == 200) {
				...
				retStr = readFile(servletContext
						.getRealPath("/defaults/themes.json"));
			}
		} catch (Exception e) {
		...
		try {
			...
			if (checkConnection.getResponseCode() != 200) {
				...
				JSONObject themesObject = new JSONObject(
						ServletUtil.streamToString(ServletUtil
								.getInputStreamForURL(themesSourceURL, "GET",
										profile)));
