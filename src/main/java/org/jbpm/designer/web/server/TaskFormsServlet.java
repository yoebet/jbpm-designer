...
public class TaskFormsServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		Definitions def = ((Definitions) unmarshaller
				.unmarshall(json, preprocessingData).getContents().get(0));

		TaskFormTemplateManager templateManager = new TaskFormTemplateManager(
				profile, packageName, assetName, getServletContext()
						.getRealPath("/" + TASKFORMS_PATH), def);
	...
	public void displayResponse(TaskFormTemplateManager templateManager,
			HttpServletResponse resp, IDiagramProfile profile) {
		try {
			StringTemplateGroup templates = new StringTemplateGroup(
					"resultsgroup", templateManager.getTemplatesPath());
			StringTemplate resultsForm = templates.getInstanceOf("resultsform");
			resultsForm.setAttribute("manager", templateManager);
			resultsForm.setAttribute("profile",
					ExternalInfo.getExternalProtocol(profile));
			resultsForm.setAttribute("host",
					ExternalInfo.getExternalHost(profile));
			resultsForm
					.setAttribute(
							"subdomain",
							profile.getExternalLoadURLSubdomain().substring(
									0,
									profile.getExternalLoadURLSubdomain()
											.indexOf("/")));
	...
	public void displayErrorResponse(HttpServletResponse resp,
			String exceptionStr) {
		try {
			ServletOutputStream outstr = resp.getOutputStream();
			resp.setContentType("text/html");
	...
	public void storeToGuvnor(TaskFormTemplateManager templateManager,
			IDiagramProfile profile) throws Exception {
		List<TaskFormInfo> taskForms = templateManager
	...
	public void storeTaskForm(TaskFormInfo taskForm, IDiagramProfile profile)
			throws Exception {
		try {
			String packagesBase = ExternalInfo
					.getExternalPackagesBasePath(profile);
			String formURL = packagesBase
					+ URLEncoder.encode(taskForm.getPkgName(), "UTF-8")
					+ "/assets/" + URLEncoder.encode(taskForm.getId(), "UTF-8");

			String createNewURL = packagesBase
					+ URLEncoder.encode(taskForm.getPkgName(), "UTF-8")
					+ "/assets/";
			...
			createConnection.setRequestProperty(
					"Slug",
					URLEncoder.encode(taskForm.getId()
							+ FORMTEMPLATE_FILE_EXTENSION, "UTF-8"));
			createConnection.setDoOutput(true);

			createConnection.getOutputStream().write(
					taskForm.getOutput().getBytes("UTF-8"));
