...
public class TaskFormsEditorServlet extends HttpServlet {
	...
	private static final Logger _logger = Logger
			.getLogger(TaskFormsEditorServlet.class);
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (action != null && action.equals(ACTION_LOAD)) {
			...
			PrintWriter pw = resp.getWriter();
			try {
				pw.write(storeTaskFormToGuvnor(taskName, packageName, profile,
						taskFormValue));
			} catch (Exception e) {
	...
	private String storeTaskFormToGuvnor(String taskName, String packageName,
			IDiagramProfile profile, String formValue) throws Exception {
		String packagesBase = ExternalInfo.getExternalPackagesBasePath(profile);
		String taskFormURL = packagesBase
				+ URLEncoder.encode(packageName, "UTF-8") + "/assets/"
				+ URLEncoder.encode(taskName, "UTF-8")
				+ TASKFORM_NAME_EXTENSION;
		...
		createConnection.setRequestProperty("Slug",
				URLEncoder.encode(taskName, "UTF-8") + TASKFORM_NAME_EXTENSION
	...
	private String getTaskFormFromGuvnor(String taskName, String packageName,
		...
		try {
			...
			String taskFormSourceURL = packagesBase
					+ URLEncoder.encode(packageName, "UTF-8") + "/assets/"
					+ URLEncoder.encode(taskName, "UTF-8")
					+ TASKFORM_NAME_EXTENSION + "/source/";
