...
public class TransformerServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		String store_only = req.getParameter("store_only");
		...
		String[] packageAssetName = ServletUtil.findPackageAndAssetInfo(uuid,
				profile);
		...
		if (transformto != null && transformto.equals(TO_PDF)) {
				...
				if (respaction != null && respaction.equals(RESPACTION_SHOWURL)) {
					...
					resp.getWriter().write(
							"<object data=\"data:application/pdf;base64,"
									+ Base64.encodeBase64(bout.toByteArray())
									+ "\" type=\"application/pdf\"></object>");
				} else {
					if (processid != null) {
						guvnorStore(packageAssetName[0], processid, profile,
								formattedSvg, rawSvg, transformto);
					}
					if ("true".equals(store_only)) {
			...
			try {
				if (respaction != null && respaction.equals(RESPACTION_SHOWURL)) {
					...
					resp.setCharacterEncoding("UTF-8");
					resp.setContentType("text/plain");
					String dataEncoded = Base64.encodeBase64String(bout.toByteArray());
					dataEncoded = dataEncoded.replaceAll("\\s", "");
					resp.getWriter().write(
							"<img src=\"data:image/png;base64," + dataEncoded + "\">");
				} else {
					if (processid != null) {
						guvnorStore(packageAssetName[0], processid, profile,
								formattedSvg, rawSvg, transformto);
					...
					if ("true".equals(store_only)) {
						...
						ParsedURL
								.registerHandler(new GuvnorParsedURLProtocolHandler(
										profile));
			...
			Definitions def = ((JbpmProfileImpl) profile)
					.getDefinitions(bpmn2in);
			List<RootElement> rootElements = def.getRootElements();
			for (RootElement root : rootElements) {
				if (root instanceof Process) {
					Process process = (Process) root;
					Iterator<FeatureMap.Entry> iter = process.getAnyAttribute()
							.iterator();
					...
					if (toDeleteFeature != null) {
						...
						EAttributeImpl extensionAttribute = (EAttributeImpl) metadata
								.demandFeature("http://www.jboss.org/drools",
						...
						EStructuralFeatureImpl.SimpleFeatureMapEntry extensionEntry = new EStructuralFeatureImpl.SimpleFeatureMapEntry(
								extensionAttribute, packageName);
			...
			String json = profile.createUnmarshaller().parseModel(
					revisedXmlModel, profile, pp);
			resp.setContentType("application/json");
			resp.getWriter().print(json);
		} else if (transformto == null && respaction != null
				&& respaction.equals(RESPACTION_SHOWEMBEDDABLE)) {
			resp.setCharacterEncoding("UTF-8");
			resp.setContentType("text/plain");
			String editorURL = ExternalInfo.getExternalRootPath(profile)
					+ "org.drools.guvnor.Guvnor/standaloneEditorServlet?assetsUUIDs="
					+ uuid + "&client=oryx";
			resp.getWriter().write(
					"<iframe id=\"processFrame\" src=\"" + editorURL
	...
	private void revisitSequenceFlows(Definitions def, String orig) {
		try {
			...
			XMLStreamReader reader = factory
					.createXMLStreamReader(new StringReader(orig));
				...
				if (reader.next() == XMLStreamReader.START_ELEMENT) {
					if ("sequenceFlow".equals(reader.getLocalName())) {
						...
						for (int i = 0; i < reader.getAttributeCount(); i++) {
							...
							if ("sourceRef".equals(reader
									.getAttributeLocalName(i))) {
								source = reader.getAttributeValue(i);
							}
							if ("targetRef".equals(reader
									.getAttributeLocalName(i))) {
						...
						if (fe instanceof SequenceFlow) {
							...
							if (sequenceFlowMapping.containsKey(sf.getId())) {
								sf.setSourceRef(getFlowNode(def,
										sequenceFlowMapping.get(sf.getId())
												.get("sourceRef")));
								sf.setTargetRef(getFlowNode(def,
										sequenceFlowMapping.get(sf.getId())
												.get("targetRef")));
							} else {
								_logger.error("Could not find mapping for sequenceFlow: "
										+ sf.getId());
							}
						}
					}
	...
	private void addBpmnDiInfo(Definitions def, String gpd) {
		try {
			...
			XMLInputFactory factory = XMLInputFactory.newInstance();
			XMLStreamReader reader = factory
					.createXMLStreamReader(new StringReader(gpd));
			while (reader.hasNext()) {
				if (reader.next() == XMLStreamReader.START_ELEMENT) {
					if ("node".equals(reader.getLocalName())) {
						...
						for (int i = 0; i < reader.getAttributeCount(); i++) {
							...
							} else if ("x".equals(reader
									.getAttributeLocalName(i))) {
								nodeX = reader.getAttributeValue(i);
							} else if ("y".equals(reader
									.getAttributeLocalName(i))) {
								nodeY = reader.getAttributeValue(i);
							} else if ("width".equals(reader
									.getAttributeLocalName(i))) {
								nodeWidth = reader.getAttributeValue(i);
							} else if ("height".equals(reader
									.getAttributeLocalName(i))) {
					...
					for (FlowElement flowElement : process.getFlowElements()) {
						if (flowElement instanceof FlowNode) {
							...
							if (sequenceFlow.getSourceRef() != null) {
								Bounds sourceBounds = _bounds.get(sequenceFlow
										.getSourceRef().getId());
								point.setX(sourceBounds.getX()
										+ (sourceBounds.getWidth() / 2));
								point.setY(sourceBounds.getY()
										+ (sourceBounds.getHeight() / 2));
							}
							edge.getWaypoint().add(point);
							// List<Point> dockers =
							// _dockers.get(sequenceFlow.getId());
							// for (int i = 1; i < dockers.size() - 1; i++) {
							// edge.getWaypoint().add(dockers.get(i));
							// }
							point = dcFactory.createPoint();
							if (sequenceFlow.getTargetRef() != null) {
								Bounds targetBounds = _bounds.get(sequenceFlow
										.getTargetRef().getId());
								point.setX(targetBounds.getX()
										+ (targetBounds.getWidth() / 2));
								point.setY(targetBounds.getY()
	...
	private void guvnorStore(String packageName, String assetName,
			IDiagramProfile profile, String formattedSvg, String rawSvg,
			String transformto) {
		try {
			String assetExt = "";
			String assetFileExt = "";
			...
			String assetsURL = packagesBase
					+ URLEncoder.encode(packageName, "UTF-8") + "/assets/";

			String pngURL = assetsURL + URLEncoder.encode(assetName, "UTF-8")
					+ assetExt;
			String deleteURL = pngURL;
			...
			_logger.info("check connection response code: "
			...
			if (checkConnection.getResponseCode() == 200) {
				...
				_logger.info("delete connection response code: "
						+ deleteConnection.getResponseCode());
			}
			// create new
			URL createURL = new URL(assetsURL);
			...
			createConnection.setRequestProperty("Slug",
					URLEncoder.encode(assetName, "UTF-8") + assetExt
							+ assetFileExt);
			...
			_logger.info("create connection response code: "
