...
public class CustomEditorsServlet extends HttpServlet {
	...
	private static final Logger _logger = Logger
			.getLogger(CustomEditorsServlet.class);
	...
	private String getCustomEditorsJSON(IDiagramProfile profile,
		...
		String customEditorsURL = packagesBase + "globalArea/assets/"
				+ CUSTOMEDITORS_NAME;

		String customEditorsSourceURL = packagesBase + "globalArea/assets/"
				+ CUSTOMEDITORS_NAME + "/source";
		String retStr = "";
		try {
			...
			if (checkConnection.getResponseCode() == 200) {
				retStr = ServletUtil.streamToString(ServletUtil
						.getInputStreamForURL(customEditorsSourceURL, "GET",
								profile));
			} else {
				retStr = readFile(servletContext
						.getRealPath("/defaults/customeditors.json"));
			}
		} catch (Exception e) {
			_logger.error("Error retriving custom editors info: "
					+ e.getMessage());
		}
