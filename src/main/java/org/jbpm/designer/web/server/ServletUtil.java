...
public class ServletUtil {
	...
	public static List<String> getFormWidgetList(IDiagramProfile profile) {
		...
		String globalAreaURL = packagesBase + "globalArea/assets";
		try {
			XMLInputFactory factory = XMLInputFactory.newInstance();
			XMLStreamReader reader = factory.createXMLStreamReader(ServletUtil
					.getInputStreamForURL(globalAreaURL, "GET", profile),
					"UTF-8");
			String title = "";
			String format = "";
			while (reader.hasNext()) {
				...
				if (next == XMLStreamReader.END_ELEMENT) {
					if ("asset".equals(reader.getLocalName())) {
						if (title.length() > 0 && format.length() > 0
								&& format.equals("fw")) {
							widgets.add(title);
							title = "";
							format = "";
	...
	public static boolean assetExistsInGuvnor(String packageName,
			String assetName, IDiagramProfile profile) {
		try {
			String packagesBase = ExternalInfo
					.getExternalPackagesBasePath(profile);
			String formURL = packagesBase
					+ URLEncoder.encode(packageName, "UTF-8") + "/assets/"
					+ URLEncoder.encode(assetName, "UTF-8");
	...
	public static boolean existsProcessImageInGuvnor(String assetURL,
			IDiagramProfile profile) {
		try {
	...
	public static List<String> getPackageNamesFromGuvnor(
			IDiagramProfile profile, String topPackageName) {
		...
		try {
			...
			while (reader.hasNext()) {
				...
				if (pname.equals(topPackageName)
						|| pname.startsWith(topPackageName + ".")) {
					packages.add(pname);
				}
			}
		} catch (Exception e) {
			_logger.error("Error retriving packages from guvnor: "
					+ e.getMessage());
		}
		return packages;
	}
	...
	public static List<String> getPackageNamesFromGuvnor(IDiagramProfile profile) {
		return getPackageNamesFromGuvnor(profile, null);
	}
	...
	public static List<String> getAllProcessesInPackage(String pkgName,
		...
		try {
			...
			String assetsURL = packagesBase
					+ URLEncoder.encode(pkgName, "UTF-8") + "/assets/";

			XMLInputFactory factory = XMLInputFactory.newInstance();
			InputStream is = ServletUtil.getInputStreamForURL(assetsURL, "GET",
					profile);
			...
			_logger.error("Error finding processes in package: "
					+ e.getMessage());
		}
		return processes;
	}

	public static String getProcessImagePath(String packageName,
			String processid, IDiagramProfile profile) {
		try {
			String packagesBase = ExternalInfo
					.getExternalPackagesBasePath(profile);
			return packagesBase + URLEncoder.encode(packageName, "UTF-8")
	...
	public static String getProcessImageSourcePath(String packageName,
			String processid, IDiagramProfile profile) {
		try {
			String packagesBase = ExternalInfo
					.getExternalPackagesBasePath(profile);
			return packagesBase + URLEncoder.encode(packageName, "UTF-8")
					+ "/assets/" + processid + "-image" + "/source/";
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
	...
	public static String getProcessSourceContent(String packageName,
			String assetName, IDiagramProfile profile) {
		try {
			String packagesBase = ExternalInfo
					.getExternalPackagesBasePath(profile);
			String assetSourceURL = packagesBase
					+ URLEncoder.encode(packageName, "UTF-8") + "/assets/"
					+ URLEncoder.encode(assetName, "UTF-8") + "/source/";

			InputStream in = ServletUtil.getInputStreamForURL(assetSourceURL,
					"GET", profile);
